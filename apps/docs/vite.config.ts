...
export default defineConfig(({ command, isPreview }) => {
  // fumadocs recrawls ~10k package.json files per config load.
  const build = command === "build" || isPreview === true
  ...
  return {
    ...
    ssr: {
      external: ["typescript", "fumadocs-typescript"],
      noExternal: build ? [/^(@fumadocs\/|fumadocs-(?!typescript$))/] : []
    },
    ...
    plugins: [
      ...
      fumadocsMdx({ updateViteConfig: !build }),
      tailwindcss(),
