...
async function startSignIn(
  auth: Awaited<ReturnType<typeof createTestServer>>["auth"],
...
describe("oauth start", () => {
  it("redirects to the provider with state and the configured redirect_uri", async () => {
    ...
    const { response, state, url } = await startSignIn(auth)
  ...
  it("sends a PKCE S256 challenge derived from the verifier it signed into the state", async () => {
    const { auth } = await createTestServer(OAUTH_OPTIONS)
    const { stateCookie, url } = await startSignIn(auth)
    const location = new URL(url)
  ...
  it("carries declared additionalFields through the body into the state", async () => {
    ...
    const { stateCookie } = await startSignIn(auth, {
  ...
  it("404s an unconfigured provider and every prototype key", async () => {
    ...
    for (const name of [
      ...
      for (const [method, path] of [
        ...
        expect((await auth.handler(request(method, path))).status, path).toBe(
          404
        )
      }
    }
...
describe("oauth redirect_uri origin", () => {
  ...
  it("derives the origin from the request when no baseURL is configured", async () => {
    const { auth } = await createTestServer(NO_BASE_URL)

    expect(await redirectURIOf(auth)).toBe(
      "https://app.example.com/api/auth/callback/github"
    )
    expect(await redirectURIOf(auth, { origin: "http://localhost:3000" })).toBe(
...
describe("oauth callback", () => {
  ...
  it("treats a guest's connect as a sign-in: merges into the linked account rather than refusing", async () => {
    ...
    const startResponse = await auth.handler(
      request("POST", "/api/auth/identities/connect/github", {
        token: await mintToken(auth, guestRefresh),
        cookies: refreshCookieFor(guestRefresh)
      })
    )
    const stateCookie = required(
  ...
  it("revalidates additionalFields from the state cookie instead of trusting them", async () => {
    ...
    const clean = await startSignIn(auth)
  ...
  it("links a verified email to an account that already signed up with a code", async () => {
    const { auth, db, sentCodes } = await createTestServer(OAUTH_OPTIONS)
    await auth.handler(
...
describe("connect and disconnect", () => {
  ...
  it("insists on the access token, like every other authenticated route", async () => {
    ...
    const withToken = await context.auth.handler(
      request("POST", "/api/auth/identities/connect/github", {
        token: await mintToken(context.auth, refreshToken)
      })
    )
...
describe("google", () => {
  ...
  async function callback(
    ...
    const { stateCookie, state } = await startGoogle(auth)
  ...
  it("sends a PKCE challenge and a nonce, and requires the nonce back in the ID token", async () => {
    const { auth } = await createTestServer(GOOGLE_OPTIONS)
    const start = await auth.handler(
      request("POST", "/api/auth/sign-in/provider/google")
    )
    const { url } = (await start.clone().json()) as { url: string }
