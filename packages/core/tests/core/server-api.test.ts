...
describe("getToken as a function", () => {
  ...
  it("accepts a Request directly, since it satisfies the headers shape", async () => {
    ...
    const result = await context.auth.getToken(
  ...
  it("answers null for a revoked session, matching the endpoint", async () => {
    ...
    await expect(
      context.auth.getToken({ headers: cookieHeaders(refreshToken) })
    ).resolves.toBeNull()
  })
  ...
  it("answers null when no cookie was sent at all", async () => {
    ...
    await expect(
      context.auth.getToken({ headers: new Headers() })
    ).resolves.toBeNull()
  })
  ...
  it("throws real Errors: instanceof, name, and a human message", async () => {
    ...
    const thrown = await context.auth
  ...
  it("serves a whole render from one exchange, touching one session", async () => {
    ...
    const updated = await context.auth.updateUser({ token, name: "Ada" })
...
describe("a token whose session is gone", () => {
  it("is refused, even though the signature still verifies", async () => {
    ...
    await expect(context.auth.deleteUser({ token })).rejects.toMatchObject({
      code: "unauthenticated",
      status: 401
    })
  })
...
describe("verifySession", () => {
  it("answers the session and its user while the session is live", async () => {
    ...
    const resolved = await context.auth.verifySession({ token })
  ...
  it("reads the bearer off headers, as every other call does", async () => {
    ...
    const resolved = await context.auth.verifySession({
  ...
  it("answers null once the session is revoked, where verifyToken does not", async () => {
    ...
    await context.auth.signOut({ token, scope: "global" })

    expect(await context.auth.verifySession({ token })).toBeNull()
    // The distinction the two calls exist to draw.
    expect(await context.auth.verifyToken(token)).not.toBeNull()
  })
  ...
  it("never answers from the cookie, so it cannot resolve another session", async () => {
    ...
    await context.auth.signOut({ token: stale.token })
    ...
    expect(await context.auth.verifySession({ headers })).toBeNull()
  })

  it("answers null for a token that does not verify at all", async () => {
    const { auth } = await createTestServer()

    expect(await auth.verifySession({ token: "not-a-token" })).toBeNull()
  })
  ...
  it("reads without writing, so checking per action never slides a session", async () => {
    ...
    await context.auth.verifySession({ token })
...
describe("signToken and decodeToken", () => {
  ...
  it("mints a service token with no subject", async () => {
    ...
    const claims = await auth.verifyToken(
      await auth.signToken({ role: "service" })
    )
  ...
  it("performs no database work", async () => {
    ...
    await context.auth.signToken({ userId: "user-1" })
  ...
  it("decodes an unverified token for triage and reports expiry", async () => {
    ...
    expect(auth.decodeToken(token)?.claims.sub).toBe("user-1")
    expect(auth.decodeToken(token)?.expired).toBe(false)
...
describe("callables and handlers agree", () => {
  it("produces the same result whether called in-process or over HTTP", async () => {
    ...
    await context.auth.sendSignInCode({ email: "ada@example.com" })
    expect(required(context.sentCodes.at(-1), "code").destination).toBe(
      "ada@example.com"
    )

    const overHttp = await context.auth.handler(
      request("GET", "/api/auth/jwks")
    )
    const inProcess = await context.auth.getJwks(undefined as never)
  ...
  it("throws a typed AuthApiError from a callable rather than returning an envelope", async () => {
    ...
    await expect(
      context.auth.updateUser({ headers: new Headers(), name: "Ada" })
...
describe("calling with a token instead of a request", () => {
  ...
  it("refuses an unreadable token when there is no cookie to fall back to", async () => {
    ...
    await expect(
      auth.updateUser({ token: "forged.not.real", name: "Ada" })
    ).rejects.toThrow()
  })
  ...
  it("serves every authenticated callable from the token alone", async () => {
    ...
    const updated = await auth.updateUser({ token, name: "Ada" })
    expect(updated.name).toBe("Ada")
    expect(updated.email).toBe("ada@example.com")
  ...
  it("names the session in the token, so bearer-only sign-out ends that session", async () => {
    ...
    expect(context.auth.decodeToken(token)?.claims.sid).toBe(
      required(row, "row").id
    )

    await context.auth.signOut({ token })
