...
export interface EndpointDefinition<Input, Data> {
   ...
   */
  run: (internals: AuthInternals, input: Input) => Promise<EndpointResult<Data>>
}
 ...
 * to describe an endpoint. From each declaration `createAuth` derives three
 * things that therefore cannot drift apart: the callable on `auth`, the
 * `Request → Response` handler, and the dispatch entry behind `auth.handler`.
 */
