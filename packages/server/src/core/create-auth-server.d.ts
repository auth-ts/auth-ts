...
type EndpointCallable<Endpoint> = Endpoint extends EndpointDefinition<infer Input, infer Data> ? (input: Input) => Promise<Data> : never;
...
export interface AuthServer extends AuthCallables {
    ...
    verifyToken: (token: string) => Promise<import("../jwt/verify-token.ts").TokenClaims | null>;
