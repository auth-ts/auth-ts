 ...
 * else's account, or have a guest's session merged into an attacker's. This is
 * the check that closes it: the browser always stamps a cross-origin `POST`
 * with `Origin`, and an origin that is not this server's own, its `baseURL`, or
 * its configured `cors.origin` is refused before the body is even parsed.
 *
 * A request with no `Origin` header passes. Browsers send it on every
 * cross-origin request and every `POST`, so its absence means a non-browser
 * client — a native app or CLI using `mode: "token"` — which holds no cookie
 * and so cannot be made to act on someone's behalf.
 *
...
export function assertAllowedOrigin(
  ...
  if (allowedOrigins(internals.options, request.url).has(origin)) return

  internals.log.warn("refused a request from a disallowed origin", { origin })
